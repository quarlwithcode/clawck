...
import { TimesheetSummary, ClawckEntry, ReportStyle, ReportBranding } from '../core/types';
...
export function generateTimesheetHTML(
  ...
  const title = branding.company_name
    ? `${branding.company_name} — ${options.title || 'Timesheet Report'}`
    : (options.title || 'Clawck Timesheet Report');
  ...
  const accentColor = branding.primary_color || '#36a2eb';
  const fontFamily = branding.font_family || "'DM Sans', sans-serif";
  const customCss = branding.custom_css || '';

  // Logo HTML (if provided)
  const logoHtml = branding.logo_base64
    ? `<img src="data:image/png;base64,${branding.logo_base64}" alt="Logo" style="max-height: 48px; margin-right: 1rem;">`
    : (branding.logo_path
      ? `<img src="${escapeHtml(branding.logo_path)}" alt="Logo" style="max-height: 48px; margin-right: 1rem;">`
      : '');
...
<body${branding.font_family ? ` style="font-family: ${escapeHtml(branding.font_family)}, sans-serif;"` : ''}>
...
${!branding.hide_clawck_branding ? `<footer style="text-align: center; color: var(--text-dim); font-size: 0.75rem; margin-top: 2rem; padding-top: 1rem; border-top: 1px solid var(--border);">
  ${branding.footer_text ? escapeHtml(branding.footer_text) : 'Generated by Clawck'}
</footer>` : (branding.footer_text ? `<footer style="text-align: center; color: var(--text-dim); font-size: 0.75rem; margin-top: 2rem; padding-top: 1rem; border-top: 1px solid var(--border);">${escapeHtml(branding.footer_text)}</footer>` : '')}
</body>
</html>`;
}
