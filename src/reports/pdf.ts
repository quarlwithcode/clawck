...
import fs from 'fs';
import { TimesheetSummary, ReportStyle, ReportBranding } from '../core/types';
...
export function generateTimesheetPDF(
  ...
  return new Promise((resolve, reject) => {
    ...
    // ─── Logo (if branding provided) ─────────────────────
    if (branding.logo_path && fs.existsSync(branding.logo_path)) {
      try {
        doc.image(branding.logo_path, 50, 50, { width: 80 });
        doc.moveDown(3);
      } catch {
        // Skip logo if it fails to load
      }
    } else if (branding.logo_base64) {
      try {
        const imgBuffer = Buffer.from(branding.logo_base64, 'base64');
        doc.image(imgBuffer, 50, 50, { width: 80 });
        doc.moveDown(3);
      } catch {
        // Skip logo if it fails to load
      }
    ...
    // ─── Header ──────────────────────────────────────────
    const headerTitle = branding.company_name
    ...
    drawHorizontalLine(doc, pageWidth, branding.primary_color);
    doc.moveDown(0.5);
    ...
    // ─── Footer ─────────────────────────────────────────
    doc.moveDown(2);
    const footerText = branding.footer_text || (branding.hide_clawck_branding ? '' : `Generated by Clawck on ${new Date().toISOString()}`);
    if (footerText) {
      doc.fontSize(8).font('Helvetica').fillColor('#888888')
...
function drawHorizontalLine(doc: PDFKit.PDFDocument, width: number, color?: string): void {
  const x = doc.x;
  const y = doc.y;
  doc.moveTo(x, y).lineTo(x + width, y).strokeColor(color || '#cccccc').stroke();
}
